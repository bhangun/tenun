...
void main() {
  const controlSeries = SimpleRugPlotSeries(
    name: 'Control',
  ...
  testWidgets('renders rug plot styles without throwing', (tester) async {
    for (final style in SimpleBarChartStyle.values) {
      ...
      expect(find.byType(SimpleRugPlotChart), findsOneWidget);
      expect(tester.takeException(), isNull);
    }
  });

  testWidgets('renders rug plot with references', (tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(
          body: SizedBox(
  ...
  testWidgets('shows rug plot tooltip on tap', (tester) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(
          body: SizedBox(
  ...
  testWidgets('invokes rug plot tap callback without tooltip', (tester) async {
    String? tappedLabel;
  ...
  testWidgets('exposes default rug plot semantics label', (tester) async {
      ...
      const MaterialApp(
        home: Scaffold(
          body: SizedBox(
            width: 460,
            height: 280,
            child: SimpleRugPlotChart(series: series),
          ),
        ),
      ),
    );
    ...
    expect(
      find.bySemanticsLabel(
        RegExp(
          r'Rug plot chart, 3 series\. Control count 10, min 34, median 66, max 96',
        ),
      ),
      findsOneWidget,
    );
